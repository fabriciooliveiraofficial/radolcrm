...
final class Exporter
{
    ...
    public function download(string $type): never
    {
        [$filename, $headers, $rows] = match ($type) {
            'clients' => ['clientes', ['Nome','Empresa','E-mail','País','Moeda','Status'], $this->db->fetchAll('SELECT name,company,email,country,preferred_currency,status FROM clients WHERE deleted_at IS NULL ORDER BY name')],
