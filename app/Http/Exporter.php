...
final class Exporter
{
    ...
    public function download(string $type): never
    {
        [$filename, $headers, $rows] = match ($type) {
            ...
            'payments' => ['pagamentos', ['Data','Cliente','Descrição','Valor original','Moeda','Cotação','Valor BRL','Taxas BRL','Líquido BRL','Status'], $this->db->fetchAll('SELECT p.payment_date,c.name,p.description,p.amount,p.currency,p.exchange_rate,p.amount_brl,p.fee_brl,p.net_brl,p.status FROM payments p JOIN clients c ON c.id=p.client_id ORDER BY p.payment_date DESC')],
