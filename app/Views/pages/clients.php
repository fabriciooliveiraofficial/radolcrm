...
if ($buFilter !== null) { $where .= ' AND c.business_unit_id=?'; $params[] = $buFilter; }
...
<?php if($showForm): ?>
<div class="modal open"><a class="modal-backdrop" href="?page=clients"></a><section class="modal-panel"><header><div><p class="eyebrow">CLIENTES</p><h2><?= $edit?'Editar cliente':'Novo cliente' ?></h2></div><a href="?page=clients" class="modal-close">×</a></header><form method="post" class="form-grid"><?= csrf_field() ?><input type="hidden" name="action" value="save_client"><input type="hidden" name="id" value="<?= (int)($edit['id']??0) ?>"><input type="hidden" name="_return" value="<?= h($_SERVER['REQUEST_URI']) ?>"><input type="hidden" name="business_unit_id" value="<?= (int)($edit ? ($edit['business_unit_id']??0) : $buFilter) ?>">
